...
export async function uploadDocuments(
  files: File[],
  categoryId?: string
): Promise<{ document_id: string; filename: string; chunk_count: number }[]> {
...
export async function replaceDocument(
  ...
  const res = await fetch(`${API_BASE}/api/documents/${documentId}/replace`, {
    method: "POST",
    headers: getAuthHeaders(),
    body: formData,
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.detail || "差し替えに失敗しました");
  }
  return res.json();
}
...
export async function getDocumentChunks(
  ...
  const res = await fetch(`${API_BASE}/api/documents/${documentId}/chunks`, {
    headers: getAuthHeaders(),
  });
...
export function sendChatMessage(
  ...
  const responsePromise = fetch(`${API_BASE}/api/chat`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
  ...
  const stream = new ReadableStream<Uint8Array>({
    async start(streamController) {
      try {
        ...
        if (!res.ok) {
          const err = await res.json().catch(() => ({}));
          throw new Error(err.detail || "チャットに失敗しました");
        }
...
export async function getChatLogs(
  ...
  const res = await fetch(
    `${API_BASE}/api/admin/logs?limit=${limit}&offset=${offset}`,
    { headers: getAuthHeaders() }
  );
...
export async function getFeedbackSummary(): Promise<FeedbackSummary> {
  const res = await fetch(`${API_BASE}/api/admin/feedback-summary`, {
    headers: getAuthHeaders(),
  });
...
export async function getPopularQuestions(
  ...
  const res = await fetch(
    `${API_BASE}/api/admin/popular-questions?limit=${limit}`,
    { headers: getAuthHeaders() }
  );
